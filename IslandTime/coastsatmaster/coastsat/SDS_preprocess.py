...
# CoastSat modules
from IslandTime.coastsatmaster.coastsat import SDS_tools
...
def create_cloud_mask(im_QA, satname, cloud_mask_issue, collection):
    ...
    #plt.hist(im_QA.flatten(), bins=100)
    #plt.show()
    im_QA_temp = np.where(im_QA < 2.3e4, 0., im_QA)
    #im_QA_temp = im_QA.copy()
    val = np.sort(np.unique(im_QA_temp[~np.isnan(im_QA_temp)]))
    #im_QA_temp = np.where((im_QA_temp == val[-1]) | 
                           #(im_QA_temp == val[-2]) | 
                          # (im_QA_temp == val[-3]) |
                           #(im_QA_temp == val[-4]) |
                           #(im_QA_temp == val[-5]), im_QA_temp, 0.)

    im_QA_no_nan = im_QA_temp[~np.isnan(im_QA_temp)]
    ...
    if satname == 'S2':
        ...
        if collection == 'C01':
            ...
            qa_values = np.unique(im_QA_no_nan.flatten())
            qa_values = qa_values.astype(int)
            cloud_values = []
