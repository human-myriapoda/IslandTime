...
def remove_duplicates(output, label):
    """
    ...
    if len(pairs) > 0:
        ...
        for pair in pairs:
            ...
            empty_bool = [(len(output[label][_]) < 2) for _ in pair]
            if np.all(empty_bool): # if both empty remove both
                ...
                if 'L9' in satnames and 'L7' in satnames: 
                    ...
                    sl0 = geometry.LineString(output[label][pair[0]]) 
                    sl1 = geometry.LineString(output[label][pair[1]])
...
def output_to_gdf(output, geomtype, label):
    """
    ...
    for i in range(len(output[label])):
        ...
        if len(output[label][i]) < 2:
            continue
        else:
            # save the geometry depending on the linestyle
            if geomtype == 'lines':
                geom = geometry.LineString(output[label][i])
            elif geomtype == 'points':
                coords = output[label][i]
