...
# CoastSat modules
from IslandTime.coastsatmaster.coastsat import SDS_tools, SDS_preprocess
...
def extract_shorelines(metadata, settings):
    ...
    for satname in metadata.keys():
        ...
        output_shoreline_waterline = []  # vector of shoreline points
        output_shoreline_vegline = []  # vector of shoreline points
        output_filename = []   # filename of the images from which the shorelines where derived
        output_cloudcover = [] # cloud cover of the images
        output_geoaccuracy = []# georeferencing accuracy of the images
        output_idxkeep = []    # index that were kept during the analysis (cloudy images are skipped)
        output_t_mndwi_waterline = []    # MNDWI threshold used to map the shoreline
        output_t_mndwi_vegline = []    # MNDWI threshold used to map the shoreline
        ...
        for i in range(len(filenames)):
            ...
            # otherwise use find_wl_contours1 (traditional)
                ...
                try: # use try/except structure for long runs
                    if sum(im_labels[im_ref_buffer,0]) < 50: # minimum number of sand pixels
                        ...
                        #contours_mwi, t_mndwi = find_wl_contours3(im_mndwi, cloud_mask, im_ref_buffer)
                        contours_mwi_waterline, contours_mwi_vegline, t_mndwi = find_wl_contours3(im_mndwi, cloud_mask, im_ref_buffer)
                    else:
                ...
                shoreline_waterline = process_shoreline(contours_mwi_waterline, cloud_mask_adv, im_nodata,
                                              georef, image_epsg, settings)
                shoreline_vegline = process_shoreline(contours_mwi_vegline, cloud_mask_adv, im_nodata,
                                              georef, image_epsg, settings)
                ...
                if settings['check_detection'] or settings['save_figure']:
                    ...
                    skip_image = show_detection(im_ms, cloud_mask, im_labels, shoreline_waterline,
                                                image_epsg, georef, settings, date, satname, otsut='shoreline')
                    skip_image = show_detection(im_ms, cloud_mask, im_labels, shoreline_vegline,
                                                image_epsg, georef, settings, date, satname, otsut='vegline')
                    # if the user decides to skip the image, continue and do not save the mapped shoreline
                    if skip_image:
                        continue
            ...
            output_shoreline_waterline.append(shoreline_waterline)
            output_shoreline_vegline.append(shoreline_vegline)
            output_filename.append(filenames[i])
            output_cloudcover.append(cloud_cover)
            output_geoaccuracy.append(metadata[satname]['acc_georef'][i])
            output_idxkeep.append(i)
            output_t_mndwi_waterline.append(t_mndwi[0])
            output_t_mndwi_vegline.append(t_mndwi[1])

        # create dictionnary of output
        output[satname] = {
                'dates': output_timestamp,
                'shorelines_waterline': output_shoreline_waterline,
                'shorelines_vegline': output_shoreline_vegline,
                'filename': output_filename,
                'cloud_cover': output_cloudcover,
                'geoaccuracy': output_geoaccuracy,
                'idx': output_idxkeep,
                'MNDWI_threshold_waterline': output_t_mndwi_waterline,
                'MNDWI_threshold_vegline': output_t_mndwi_vegline
                }
        print('')
...
def find_contours_mpe(im_ndwi, im_ndvi, im_nir, cloud_mask, im_ref_buffer):
    ...
    t_otsu = filters.threshold_multiotsu(vec, classes=3)
    # use Marching Squares algorithm to detect contours on ndwi image
    im_ndwi_buffer = np.copy(im_ndwi)
    im_ndwi_buffer[~im_ref_buffer] = np.nan
    contours_waterline = measure.find_contours(im_ndwi_buffer, t_otsu[0], fully_connected='high', positive_orientation='high')
    contours_vegline = measure.find_contours(im_ndwi_buffer, t_otsu[1], fully_connected='high', positive_orientation='high')
    # remove contours that contain NaNs (due to cloud pixels in the contour)
    contours_waterline = process_contours(contours_waterline)
    contours_vegline = process_contours(contours_vegline)

    return contours_waterline, contours_vegline, t_otsu

def find_wl_contours3(im_ndwi, cloud_mask, im_ref_buffer):
...
def show_detection(im_ms, cloud_mask, im_labels, shoreline,image_epsg, georef,
    ...
    if settings['save_figure'] and not skip_image:
        ...
        fig.savefig(os.path.join(filepath, date + '_' + satname + '_{}.jpg'.format(otsut)), dpi=150)
...
def adjust_detection(im_ms, cloud_mask, im_nodata, im_labels, im_ref_buffer, image_epsg, georef,
    ...
    try:
        if sum(sum(im_labels[:,:,0])) > 50:
            # use classification to refine threshold and extract the sand/water interface
            contours_mndwi, t_mndwi = find_wl_contours3(im_mndwi, cloud_mask, im_ref_buffer)
        else:       
            # find water contours on MNDWI grayscale image
            contours_mndwi, t_mndwi = find_wl_contours3(im_mndwi, cloud_mask, im_ref_buffer)    
    except:
