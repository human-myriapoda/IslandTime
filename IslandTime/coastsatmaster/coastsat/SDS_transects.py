...
def compute_intersection(output, transects, settings, label):
    """
    ...
    intersections = np.zeros((len(output[label]),len(transects)))
...
def reject_outliers(cross_distance, output, settings, label):
    """
...
#        satnames1 = [output['satname'][k] for k in idx_nonan]
        ...
        if np.isnan(settings['otsu_threshold'][0]):
            ...
            threshold1 = [output['MNDWI_threshold_{}'.format(label)][k] for k in idx_nonan]
