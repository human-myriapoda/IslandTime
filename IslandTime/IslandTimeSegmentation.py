...
class Segmentation:
    def __init__(self, island_info, list_sat=['S2', 'L8', 'L9'], generate_unsupervised_classification=False, overwrite=False):
    ...
    def _coregistration_PSS(self, rgb, nir, visual_inspection=True, return_geotransform=False):
        ...
        for ws in ws_list:
            try:
                CRL = COREG(rgb, nir, align_grids=True, max_shift=100, ws=(ws, ws), max_iter=100, q=True, verbose=False)
    ...
    def _combine_contours(self, contours, img, verbose, min_len=100, plot_int_debug=False):
            ...
            if i not in to_exclude:
                ...
                for j in range(len(contours_open)):
                    if i != j and j not in to_exclude:
                        ...
                        lines = self.combine_geometry_close_endpoints(line1, line2)
        ...
        for con in range(len(contours_open)):
            contours_open[con] = self.combine_geometry_extent(contours_open[con], img)
    ...
    def _band_segmentation(self, im, rgb, label, segmented_image, cmap, classes_to_consider=None, plot_results=False, animation=False, verbose=False, find_polygons=True):
        ...
        for idx_minimum, minimum in enumerate(list(reversed(thresholds))):
            ...
            contours = measure.find_contours(im, minimum, fully_connected='high', positive_orientation='high')

            # Get Otsu contour if label is NIR, NDWI and NDVI
            if label in ['NIR', 'NDWI', 'NDVI', 'NDWI', 'red', 'green', 'blue']:
                contours_otsu = measure.find_contours(im, f_otsu_two_classes, fully_connected='high', positive_orientation='high')
    ...
    def _return_polygons_coi(self, polygons_dict, center_of_image):
        # Empty lists
        polygons_coi = []
        thresholds_coi = []
    ...
    def _find_optimal_polygon(self, polygons_dicts, center_of_image, rgb, plot_results=True, metrics='length'):
        
        dict_NIR = polygons_dicts[0]
        dict_NDVI = polygons_dicts[1]
        dict_NDWI = polygons_dicts[2]

        poly_NDWI, threshold_NDWI = self._return_polygons_coi(dict_NDWI, center_of_image)
        poly_NDVI, threshold_NDVI = self._return_polygons_coi(dict_NDVI, center_of_image)
        poly_NIR, threshold_NIR = self._return_polygons_coi(dict_NIR, center_of_image)
    ...
    def extract_polygons(self):
        ...
        if os.path.exists(all_polygons_file) and not self.overwrite:
            # Read file
            all_dicts = pickle.load(open(all_polygons_file, 'rb'))
        ...
        metadata = pickle.load(open(os.path.join(os.getcwd(), 'data', 'coastsat_data', '{}_{}'.format(self.island, self.country), '{}_{}_metadata.pkl'.format(self.island, self.country)), 'rb'))
        ...
        for sat in self.list_sat:
            ...
            for idx_filename, filename in enumerate(tqdm(filenames)):
                if sat == 'PSS':
                    ...
                    nir_corrected = self.coregistration_PSS(rgb, nir)
                    nir = nir_corrected
                    ...
                    ndwi = SDS_tools.nd_index(im_ms[:, :, 1], im_ms[:, :, 3], cloud_mask)

                # Create stacked arrays
                stacked_array = np.dstack((blue, green, red, nir, ndvi, ndwi))
                stacked_array_opt = np.dstack((nir, ndvi, ndwi))
                ...
                res_optimal = self._find_optimal_polygon(list_dict_polygons_segmented, centre_image, rgb, plot_results=True)
    ...
    def find_best_polygons(self, dict_polygons_ts, dict_rgb_ts):
        ...
        if os.path.exists(best_polygons_file) and not self.overwrite:
            # Read file
            dict_best_polygons = pickle.load(open(best_polygons_file, 'rb'))
    ...
    def time_series_from_polygons(self, dict_poly, dict_georef, dict_image_epsg, dict_best_polygons):
        ...
        for key in dict_best_polygons.keys():
            ...
            for key_transect, transect in transects_image_crs.items():
                ...
                point_transect = shapely.geometry.Point(self.transects[0][1, 0], self.transects[0][1, 1])
    ...
    def main(self):
        ...
        dict_time_series = self.time_series_from_polygons(dict_polygons_ts, dict_georef_ts, dict_image_epsg_ts, dict_best_polygons)
